...
# from ha_mqtt_discoverable import Discoverable, Settings, __version__
# from ha_mqtt_discoverable.utils import clean_string, valid_configuration_key

# TODO it this class still required?
...
#         # self.topic_prefix = f"{settings.mqtt.mqtt_prefix}/sensor/{clean_string(self.device_name)}-{clean_string(self.unique_id)}"
#         # self.config_topic = f"{self.topic_prefix}/config"
#         # self.state_topic = f"{self.topic_prefix}/state"

#         logging.debug(f"device_name: {self._settings.device.name}")
...
#         logging.debug(f"unique_id: {self._settings.unique_id}")
...
#         """
#         return self._settings.device.dict()
